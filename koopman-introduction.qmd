...
format:
  html:
    ...
    include-in-header:
      text: |
        <!-- Centralized color scheme for recurring symbols: change the hex
             codes here to re-theme every occurrence of psi, Psi, lambda, v
             and g across the whole document. Note: we cannot override the
             built-in \psi / \Psi / \lambda macros directly - MathJax treats
             that as redefining a primitive and breaks parsing for any math
             after the first use, so we define new command names instead and
             use those throughout the body. -->
        <script>
        window.MathJax = {
          tex: {
            macros: {
...
$$
\kg(\Phi_0) = \sum_{j=0}^{J} \kv_j \kpsi_j(\Phi_0)
$$
...
Now that the initial condition is expressed in the basis of eigenfunctions, we can propagate the signal in this space :

$$
K^t \kg(\Phi_0) = \sum_{j=0}^{\infty} \kv_j K^t \kPsi_j(\Phi_0) = \sum_{j=0}^{\infty} \kv_j\, e^{j\klam t}\, \Phi_0^j = \kg(T(\Phi_0, t))
$$

In practice $j$ is finite, so the last equality sign is an approximation !
